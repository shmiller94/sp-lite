...
export const AreasToImproveStep = () => {
  const { next, areasToImprove, totalCategoryCount } =
  ...
  const hasAreasToImprove = areasToImprove.length > 0;

  return (
    <ProtocolStepLayout>
      <div>
        <H2 className="mb-1 text-center md:text-3xl">Your areas to improve</H2>
        <Body1 className="mb-6 text-balance text-center text-secondary">
          {hasAreasToImprove
            ? `${areasToImprove.length} out of ${totalCategoryCount} health categories require some extra attention.`
            : 'All categories are doing well!'}
        </Body1>
      </div>
      <div className="relative max-w-md space-y-8 rounded-2xl bg-gradient-to-t from-white via-white to-pink-50/20 p-6 pt-0 shadow shadow-black/[.03] duration-700 ease-out animate-in fade-in slide-in-from-bottom-4">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2">
          {hasAreasToImprove ? (
            areasToImprove.map((area) => (
              ...
              >
                <MiniScoreChart value={area.value} />
                <p className="text-secondary">{area.category}</p>
              </div>
            ))
          ) : (
            <p className="text-secondary">
              Great job! No areas need attention.
            </p>
          )}
        </div>
      </div>
