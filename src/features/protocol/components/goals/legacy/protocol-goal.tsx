...
import type { LegacyActivity, LegacyGoal } from '../../../api';
...
type LegacyProtocolGoalProps = {
  goal: LegacyGoal;
  activities: LegacyActivity[];
  allGoals?: LegacyGoal[];
  tempSelections?: Set<string>;
  onSelectionChange?: (activityKey: string, selected: boolean) => void;
  getActivityKey?: (activity: LegacyActivity, index: number) => string;
};

export function LegacyProtocolGoal({
  goal,
  activities,
  allGoals,
  tempSelections,
