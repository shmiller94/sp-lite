...
import { useLegacyLatestProtocol, useLegacyProtocol } from '../../../api';
import { getGoalIndex } from '../../../utils/get-goal-index';
import { ProtocolLayout } from '../../layouts/protocol-layout';

import { LegacyProtocolGoal } from './protocol-goal';

type LegacyProtocolGoalViewProps = {
  protocolId: string;
  goalId: string;
};

/**
 * View for a specific goal and its activities in a legacy protocol
 * Path: /protocol/legacy/:planId/goals/:goalId
 */
export function LegacyProtocolGoalView({
  protocolId,
  goalId,
}: LegacyProtocolGoalViewProps) {
  const { data: lastProtocol, isLoading: isLatestProtocolLoading } =
    useLegacyLatestProtocol();
  const { data: protocol, isLoading, error } = useLegacyProtocol(protocolId);
  const navigate = useNavigate();

  const protocolLink =
    lastProtocol?.id === protocolId
      ? '/protocol'
      : `/protocol/legacy/${protocolId}`;
  ...
  const handleNextGoal = () => {
    if (hasNextGoal) {
      navigate({
        to: `/protocol/legacy/${protocolId}/goals/${nextGoalId}` as any,
      });
    } else {
      navigate({ to: protocolLink as any });
    }
  };
