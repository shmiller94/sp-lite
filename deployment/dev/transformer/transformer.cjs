...
/******/ (() => {
  ...
  this.result;*/
    ...
    /***/ 379: /***/ function (
      ...
      const yaml = __importStar(__webpack_require__(210));
      const child_process_1 = __webpack_require__(200);
      function processResource(resource) {
        var _a, _b, _c, _d;
        const project =
          (_b =
            (_a = resource.metadata) === null || _a === void 0
              ? void 0
              : _a.annotations) === null || _b === void 0
            ? void 0
            : _b["doppler.com/project"];
        const config =
          ((_d =
            (_c = resource.metadata) === null || _c === void 0
              ? void 0
              : _c.annotations) === null || _d === void 0
            ? void 0
            : _d["doppler.com/config"]) || "dev";
        if (!project) return resource;
        // Substitute Doppler secrets
        try {
          // Convert resource to YAML string
          const resourceYaml = yaml.dump(resource);
          const processedYaml = resourceYaml.replace(
            /:\s*'({{tojson\s+[^}]+}})'/,
            ": $1"
          );
          // Create a temporary file for the resource
          const tempFilePath = `/tmp/resource-${Math.random()
            .toString(36)
            .substring(2, 7)}.yaml`;
          fs.writeFileSync(tempFilePath, processedYaml);
          // Prepare Doppler command with potential project/config params
          let dopplerCommand = `doppler secrets substitute --project ${project} --config ${config} ${tempFilePath}`;
          // Execute Doppler substitute command
          const substitutedYaml = (0, child_process_1.execSync)(
            dopplerCommand,
            { encoding: "utf-8" }
          );
          // Clean up temporary file
          fs.unlinkSync(tempFilePath);
          // Parse the substituted YAML back to an object
          const substitutedResource = yaml.load(substitutedYaml);
          return substitutedResource;
        } catch (error) {
          console.error(`Error substituting secrets: ${error.message}`);
          // If Doppler fails, return the original resource
          return resource;
        }
      }
      /**
       * Main function to transform Kubernetes resources
       */
      async function transform() {
        try {
          // Read config from stdin
          const stdinBuffer = fs.readFileSync(0, "utf-8");
          const inputYaml = yaml.loadAll(stdinBuffer);
          let resources = inputYaml[0].items;
          // Find our configuration in the input
          // const config = inputYaml.find(
          //   (doc) =>
          //     doc.kind === "Transformer" &&
          //     doc.apiVersion === "kustomize.config.k8s.io/v1",
          // ) as TransformerConfig;
          //
          // if (!config) {
          //   throw new Error("Transformer configuration not found in input");
          // }
          // Extract resources to transform (filtering out the transformer config itself)
          ...
          // Output transformed resources as YAML
          const output = transformedResources
            .map((res) => yaml.dump(res))
            .join("---\n");
    ...
    /***/ 489: /***/ (
      ...
      module.exports = __webpack_require__(769).extend({
        implicit: [__webpack_require__(127), __webpack_require__(851)],
        explicit: [
